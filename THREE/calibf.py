...
class calibf():
    ...
    def fitExpt(self,fNPE,model='DEFAULT',muMC=None,PLOT=True,allSteps=False,useLogLike=False,debug=-1,comment=''):
        ...
        words = 'fitExpt_meanData_{:}_meanMC_{:}permille'.format(int(1000*meanmu),int(1000*muMC)) + comment
        ...
        if PLOT : 
            ...
            ax1.set_xlim(xlim) #(max(0.,bfCF-.2),bfCF+.2))
            ...
            ax2.set_xlim(xlim) #(max(0.,bfCF-.1),bfCF+.1))
    ...
    def writeStats(self,a,ax,xfactor=0.6, yfactor=0.8, words=''):
        ...
        return
    def oneExpt(self,muData=0.63,muMC=0.5,Nevt=1000,PLOT=True,allSteps=False,useLogLike=False,debug=-1,comment=''):
        '''
        ...
        bfNPE, bfCF = self.fitExpt(fNPE, model='DEFAULT', muMC=muMC, PLOT=PLOT, allSteps=allSteps, useLogLike=useLogLike,debug=debug,comment=comment)
    ...
    def plotExptData(self,exptData):
        ...
        #words = 'main_Nexpt_{:}_Nevt_{:}'.format(Nexpt,Nevt)
        ...
        FIG, AX = plt.subplots(nrows=nrows,ncols=ncols,sharex=True,constrained_layout=True, figsize=(1.1*6.4,1.3*4.8)) # (6.4,4.8) =default size in inches width, height matplot3.5.1
        ...
        for iCF,trueCF in enumerate(trueCFlist):
            ...
            for iConfig in exptData:
                ...
                if trueCF==tCF :
                    ...
                    #print('iax',iax,'iCF',iCF,'iConfig',iConfig,label)
        ...
        for a in [ax0]:
            ...
            ylim = [-yma,yma]
            a.set_ylim( ylim )
        # specifying my favorite tick spacing
    ...
    def readAndPlot(self,timestamps):
        '''
        read in pickle file specified by timestamps and plot contents
        '''
